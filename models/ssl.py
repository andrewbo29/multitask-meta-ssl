...
import utils.spsa_optimizer as spsa_optimizer
...
class SimCLR(BaseSSL):
    ...
    def add_model_hparams(cls, parser):
        # loss params
        parser.add_argument('--reg', default=50.0, type=float, help='Reg used in R2D2 loss')
        ...
        parser.add_argument('--rottlr', default=-1, type=float, help='regularizer of using rotation prediction loss')
        parser.add_argument('--rottlr_p', default=-1, type=float, help='probablity of rotation when used as task augmentation')
        parser.add_argument('--ori_reg', default=1, type=float, help='regularizer of using original ssl loss, i.e., contrastive loss')
        parser.add_argument('--mix_reg', default=1, type=float, help='regularizer of using cutmix loss')
        # data params
        parser.add_argument('--multiplier', default=2, type=int)
        parser.add_argument('--rot_div', default=1, type=int, help='part of the data used to rotate')
        ...
        #mtm
        parser.add_argument('--mtm', action='store_true')
        parser.add_argument('--tasks_number', type=int)
        parser.add_argument('--mtm_steps', type=int)

        parser.add_argument('--multi_task', action='store_true')
        parser.add_argument('--task_multiplier', type=int, default=4)
    ...
    def __init__(self, hparams, device=None):
        ...
        ## adding the loss for rotation prediction
        ...
        if self.hparams.head == 'contrastive':
            ...
            #     )
            if self.hparams.mtm:
                self.criterion = models.losses.NTXent_PN_Fixed(
                    n_supp=hparams.n_supp,
                    n_query=hparams.n_query,
                    tau=hparams.temperature,
                    multiplier=hparams.multiplier / (hparams.mtm_steps + 1),
                    distributed=(hparams.dist == 'ddp'),
                )
            else:
                self.criterion = models.losses.NTXent_PN_Fixed(
            ...
            self.test_criterion = self.criterion
        elif self.hparams.head == 'SVM':
            self.criterion = models.losses.NTXent_SVM(
                    bs=hparams.bs,
                    n_supp=hparams.n_supp,
                    n_query=hparams.n_query,
                    tau=hparams.temperature,
    ...
    def step(self, batch, it):
        ...
        if self.hparams.rottlr > 0 and self.hparams.qcm:
            ...
            #kks = list(np.arange(x.shape[0]))
            #kk = random.sample(kks, int(x.shape[0]/self.hparams.rot_div))
            #x_ = x[kk]
            #x_rots = []
            #rotate_labels = [1,2,3]
            #for k in rotate_labels:
            #    #x_rots.append(torch.rot90(x, k, [2, 3]))
            #    x_rots.append(torch.rot90(x_, k, [2, 3]))
            #x_rots = torch.cat(x_rots)
            #x_rots = torch.cat([x_, x_rots])
            #z = self.model(x)
            #z_rots = self.model(x_rots, out='rot')
            ...
            ###loss for large rotation
            #loss_rot = self.criterion_rot(z_rots)

            ## generate rotated images and their labels
            x_rots = []
            rot_len = int(x.shape[0]/self.hparams.rot_div)
            ks = torch.tensor(np.random.randint(4, size=rot_len)).cuda()
            for i in range(rot_len):
        ...
        ## task augmentation 
            ...
            if self.hparams.multi_task:
                task_weight = 1 / self.hparams.task_multiplier
                task_indexes = self.hparams.batch_size * self.hparams.multiplier
                loss = 0
                acc = 0
                for task_ind in range(self.hparams.task_multiplier):
                    task_z = z[task_indexes * task_ind:task_indexes * (task_ind + 1)]
                    task_loss, task_acc = self.criterion(task_z)
                    loss += task_weight * task_loss
                    acc += task_acc
                acc /= self.hparams.task_multiplier
            elif self.hparams.mtm:
                cls_inds = np.random.permutation(self.hparams.batch_size)
                tasks_inds = []
                cls_number_in_task = int(self.hparams.batch_size / self.hparams.tasks_number)
                for i in range(self.hparams.tasks_number):
                    tasks_inds.append(cls_inds[i * cls_number_in_task:(i + 1) * cls_number_in_task])

                # print(len(tasks_inds), len(tasks_inds[0]))

                all_tasks_inds = []
                for task in tasks_inds:
                    inds = []
                    for k in range(self.hparams.multiplier):
                        for j in task:
                            inds.append(j + k * self.hparams.batch_size)
                    all_tasks_inds.append(inds)

                size = cls_number_in_task * (self.hparams.n_supp + self.hparams.n_query)
                # mtm_weights = [1 / self.hparams.tasks_number] * self.hparams.tasks_number
                mtm_weights = np.array([1 / self.hparams.tasks_number for _ in range(self.hparams.tasks_number)])
                # mtm_weights = np.ones(self.hparams.tasks_number)
                init_mtm_weights = mtm_weights.copy()
                # print(init_mtm_weights)
                with torch.no_grad():
                    for i in range(self.hparams.mtm_steps):
                        # sum_loss = 0
                        losses = []
                        for task_ind in range(self.hparams.tasks_number):
                            # print(len(all_tasks_inds[task_ind]), i * size, (i + 1) * size)
                            inds = all_tasks_inds[task_ind][i * size:(i + 1) * size]
                            # print(len(inds))
                            loss_task, _ = self.criterion(z[inds])
                            # print(loss_task.item())
                            losses.append(loss_task)
                            # sum_loss += mtm_weights[task_ind] * loss_task
                        mtm_weights = spsa_optimizer.optimize(mtm_weights, losses, i + 1)

                mtm_weights_delta = abs((init_mtm_weights - mtm_weights).mean())
                # print(mtm_weights_delta)
                # print(mtm_weights)

                loss = 0
                acc = 0
                for task_ind in range(self.hparams.tasks_number):
                    inds = all_tasks_inds[task_ind][self.hparams.mtm_steps * size:(self.hparams.mtm_steps + 1) * size]
                    loss_task, acc_task = self.criterion(z[inds])
                    # print(loss_task.item(), acc_task.item())
                    loss += mtm_weights[task_ind] * loss_task
                    # loss += init_mtm_weights[task_ind] * loss_task
                    acc += acc_task

                acc /= self.hparams.tasks_number
    ...
    def train_step_maxup(self, batch, it=None):
        logs = self.step_maxup(batch)
    ...
    def samplers(self):
        ...
        if self.hparams.multi_task:
            batch_sampler.MULTILPLIER = self.hparams.multiplier * self.hparams.task_multiplier
...
class SSLEval(BaseSSL):
    ...
    def __init__(self, hparams, device=None):
        ...
        if hparams.data == 'cifar':
            hdim = self.encode(torch.ones(80, 3, 32, 32).to(device)).shape[1]
            n_classes = 10
        elif hparams.data == 'imagenet':
            hdim = self.encode(torch.ones(80, 3, 224, 224).to(device)).shape[1]
            n_classes = 1000
        ...
        if len(self.trainset) != self.hparams.train_size:
            if self.hparams.data == 'cifar' or self.hparams.data == 'cifar100':
                ...
                if isinstance(self.trainset, torch.utils.data.TensorDataset):
                    ...
                    train_files = urllib.request.urlopen(f'https://raw.githubusercontent.com/google-research/simclr/master/imagenet_subsets/{self.hparams.train_size}percent.txt').readlines()
                    samples = []
