...
class NTXent_PN_Fixed(nn.Module):
    """
    Contrastive loss with distributed data parallel support
    """
    LARGE_NUMBER = 1e9
    ...
    def forward(self, z, get_map=False):
        ...
        support_labels = torch.LongTensor(torch.arange(n_way)).unsqueeze(0).cuda()
        support_labels = support_labels.repeat(1, self.n_supp)

        # print(support_labels.shape)

        rand = torch.rand(1, self.n_supp*n_way)
        rand_supp = rand.argsort(dim=1)
        support[0] = support[0, rand_supp[0], ...]
        support_labels[0] = support_labels[0, rand_supp[0]]

        query_labels = torch.LongTensor(torch.arange(n_way)).unsqueeze(0).cuda()
        query_labels = query_labels.repeat(1, self.n_query)

        rand = torch.rand(1, self.n_query*n_way)
        rand_query = rand.argsort(dim=1)
        query[0] = query[0, rand_query[0], ...]
        query_labels[0] = query_labels[0, rand_query[0]]

        # print(support.shape, query.shape, support_labels.shape, query_labels.shape)

        ### Updates
        # support = support.reshape(1, self.n_supp*n_way, -1)
        # query = query.reshape(1, self.n_query*n_way, -1)
        # support_labels = support_labels.repeat(1, self.n_supp)
        # query_labels = query_labels.repeat(1, self.n_query)
        # print(support.shape, query.shape, support_labels.shape)
        ###

        # print(support.shape, query.shape, support_labels.shape, query_labels.shape, n_way, self.n_supp)

        logits = ProtoNetHead(query, support, support_labels, n_way, self.n_supp, normalize=False).squeeze()

        ### Updates
        # print(logits.shape)
        # logits = logits.reshape(n_way, -1)
        # print(logits.shape)
